...
export async function GET(request: NextRequest) {
  ...
  if (!code) {
    ...
    authUrl.searchParams.set(
      "redirect_uri",
      `${request.nextUrl.origin}/api/eve/link`,
    );
    authUrl.searchParams.set("client_id", env.EVE_CLIENT_ID);
    authUrl.searchParams.set(
      "scope",
      "esi-characters.read_characters.v1 esi-corporations.read_corporation_membership.v1 esi-wallet.read_character_wallet.v1 esi-markets.read_character_orders.v1",
    );
  ...
  try {
    ...
    if (characterInfoResponse.ok) {
      ...
      if (corpResponse.ok) {
        const corpData = (await corpResponse.json()) as {
          name: string;
          alliance_id?: number;
        };
        ...
        if (corpData.alliance_id) {
          ...
          if (allianceResponse.ok) {
            const allianceData = (await allianceResponse.json()) as {
              name: string;
            };
    ...
    return NextResponse.json(
      { error: "Failed to link EVE character" },
      { status: 500 },
    );
  }
}
